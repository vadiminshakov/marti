...
func Get() (pair entity.Pair, klinesize string, koeff, usebalance, minwindow decimal.Decimal, _ error) {
	...
	if err != nil {
		return entity.Pair{}, "", decimal.Decimal{}, decimal.Decimal{}, decimal.Decimal{}, err
	}
	...
	if err != nil {
		return entity.Pair{}, "", decimal.Decimal{}, decimal.Decimal{}, decimal.Decimal{}, fmt.Errorf("invalid --klinesize provided: --klinesize=%s", klinesize)
	}

	ub := usebalance.BigInt().Int64()

	if ub < 0 || ub > 100 {
		return entity.Pair{}, "", decimal.Decimal{}, decimal.Decimal{}, decimal.Decimal{}, fmt.Errorf("invalid --usebalance provided, --usebalance=%f", usebalance)
	}

	pairElements := strings.Split(*pairFlag, "_")
	if len(pairElements) != 2 {
		return entity.Pair{}, "", decimal.Decimal{}, decimal.Decimal{}, decimal.Decimal{}, fmt.Errorf("invalid --par provided, --pair=%s", *pairFlag)
	}
	pair = entity.Pair{From: pairElements[0], To: pairElements[1]}

	return pair, klinesize, koeff, usebalance, minwindow, nil
}
